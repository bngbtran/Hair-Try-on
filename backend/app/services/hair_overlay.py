...
from app.services.mediapipe_models import face_landmarker
...
def _face_geometry(img_rgb: np.ndarray) -> dict | None:
    ...
    return dict(
        lm=lm, W=W, H=H,
        face_width=face_width, cheek_width=cheek_width,
        temple_left=temple_left, temple_right=temple_right,
        angle_deg=ang, eye_center=ec,
        brow_y=brow_y, nose_tip_y=nose_tip_y,
        hairline_pt=hairline_pt, skin_color=skin_color,
        yaw_ratio=yaw_ratio,
...
def _refine_root_y(
    ...
    y0 = max(1, approx_y - search_radius);  y1 = min(H, approx_y + search_radius)
...
def _face_anchored_placement(
    ...
    if len(ys_h) == 0:
        ...
        return float(hp[0]), float(hp[1]), w_h // 2, h_h // 2, 1.0, angle_deg, float(hp[1])
    ...
    brow_y = geo["brow_y"]
    lm10_y = lm10[1]
    mask_root_y, mask_cx, mask_width, mask_top, mask_height = _mask_geometry(hair_mask, geo)

    is_fringe = (
        (mask_top - lm10_y) > (brow_y - lm10_y) * 0.2
        if mask_root_y is not None else False
    )
